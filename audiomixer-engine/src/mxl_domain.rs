//! (Shared with mxl-bridge's src/mxl_domain.rs - keep the two in step.)
//!
...
pub fn load_or_create(domain_dir: &Path, label: &str) -> anyhow::Result<DomainDef> {
    ...
    if !path.exists() {
        ...
        let def = DomainDef {
            ...
            description: format!("MXL domain at {} (definition created by audiomixer-engine)", domain_dir.display()),
            tags: BTreeMap::new(),
        };
